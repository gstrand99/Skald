...
pub(crate) async fn toggle(
    ...
    match toggle_decision(&job_state) {
        ...
        ToggleDecision::Busy => state_error(request_id, &state, "busy", "Skald is busy").await,
    }
}
...
pub(crate) async fn finish_dictation(
    ...
    if !recording.speech_detected {
        if state.notifications.enabled && state.audio_gates.notify_on_no_speech {
            skald_platform::notify("Skald", "No speech detected");
        }
        clear_per_job_context(state).await;
        let status = update_state(state, None, JobState::Idle).await;
    ...
    if transcript.text.trim().is_empty() {
        if state.notifications.enabled {
            skald_platform::notify("Skald", "No speech recognized");
        }
        clear_per_job_context(state).await;
        let status = update_state(state, None, JobState::Idle).await;
        if diagnostics_enabled {
    ...
    let routing = skald_core::routing::resolve_cleanup_routing(
...
pub(crate) async fn cleanup_preview(
    ...
    if !cleanup_enabled && cleanup_config.provider == "none" {
        return state_error(
            ...
            state,
            "cleanup_disabled",
            "cleanup is disabled; run skald cleanup enable openrouter",
        )
        .await;
    }
    let routing = skald_core::routing::resolve_cleanup_routing(
...
pub(crate) fn apply_voice_commands(
    ...
    let Ok(registry) = skald_core::commands::build_command_registry(paths_config) else {
        tracing::debug!("voice command registry unavailable");
        return outcome;
    };
    let Some(parsed) = skald_core::commands::parse_voice_command(
        voice_commands_config,
        &registry,
        transcript.trim(),
    ) else {
    ...
    match parsed.target {
        skald_core::commands::CommandTarget::Style { name } => {
            outcome.voice_style = Some(name);
        }
        skald_core::commands::CommandTarget::Snippet { name } => {
            match skald_core::snippets::snippet_kind(paths_config, &name) {
                Ok(skald_core::snippets::SnippetKind::Insert)
                    if outcome.raw_text.trim().is_empty() =>
                {
                    outcome.insert_snippet_only = Some(name);
                }
                Ok(skald_core::snippets::SnippetKind::Template)
                    if !outcome.raw_text.trim().is_empty() =>
                {
...
pub(crate) async fn deliver_template_from_job(
    ...
    let metadata = match skald_core::snippet_templates::load_template_metadata(&paths_config, &name)
    {
...
pub(crate) async fn deliver_snippet_from_job(
    ...
    let snippet_text = match skald_core::snippets::load_snippet_content(&paths_config, &name) {
...
pub(crate) async fn insert_snippet(request_id: String, state: &AppState, name: String) -> Response {
    ...
    *state.target_at_start.lock().await = target_at_insert.clone();
    ...
    let content = match skald_core::snippets::load_snippet_content(&paths_config, &name) {
        Ok(content) => content,
        Err(error) => {
            clear_per_job_context(state).await;
            let status = update_state(state, None, JobState::Idle).await;
