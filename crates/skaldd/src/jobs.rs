...
pub(crate) struct AppState {
    ...
    pub(crate) active_app_profile: Mutex<Option<skald_core::apps::AppProfile>>,
...
pub(crate) async fn audio_error_response(
    ...
    if state.notifications.enabled && matches!(&error, audio::AudioError::StreamFailed { .. }) {
        skald_platform::notify("Skald", "Recording failed: audio stream error");
    }
...
pub(crate) fn current_session_environment() -> SessionEnvironment {
    let report = skald_platform::environment_report();
    SessionEnvironment {
...
mod tests {
    ...
    use super::*;
    use skald_core::config::{AsrConfig, AudioConfig, PreviewConfig, VocabularyConfig};
