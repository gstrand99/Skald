...
pub(crate) async fn capture_active_target_async() -> Option<skald_platform::TargetContext> {
    tokio::task::spawn_blocking(skald_platform::capture_active_target)
        .await
        .ok()
        .flatten()
}
...
pub(crate) fn prefer_clipboard_for_target(
    target: Option<&skald_platform::TargetContext>,
    paths: &PathsConfig,
) -> bool {
    target
        .and_then(|target| {
            skald_core::apps::match_app_profile(
                paths,
...
pub(crate) async fn deliver_text_to_target(
    ...
    ) && let Some(snapshot) = clipboard_snapshot
    {
        ...
        tokio::task::spawn_blocking(move || skald_platform::wait_for_clipboard(delay_ms))
            .await
            .ok();
        match tokio::task::spawn_blocking(move || skald_platform::restore_clipboard(snapshot)).await
        {
            Ok(Ok(())) => true,
            Ok(Err(error)) => {
...
pub(crate) async fn copy_final_text(
    ...
    tokio::task::spawn_blocking(move || {
        skald_platform::copy_to_clipboard(&text).map_err(|error| {
            let message = error.to_string();
            if notifications_enabled {
                skald_platform::notify("Skald clipboard failed", &message);
            }
            message
        })
    })
...
pub(crate) fn handle_clipboard_fallback(
    ...
    {
        skald_platform::notify("Skald clipboard only", &outcome.insertion_reason);
    }
    outcome
}

pub(crate) async fn test_clipboard(request_id: String, state: &AppState) -> Response {
    let test_value = format!("Skald clipboard test {}", now_ms());
    let result = tokio::task::spawn_blocking(move || {
        let snapshot = skald_platform::save_clipboard();
        let result = skald_platform::copy_to_clipboard(&test_value)
            .and_then(|()| skald_platform::read_clipboard())
            .and_then(|value| {
                if value == test_value {
                    Ok(())
                } else {
                    Err(skald_platform::PlatformError::InvalidOutput {
                        tool: "clipboard",
                        message: "clipboard contents did not match".into(),
                    })
                }
            });
        let restore_result = skald_platform::restore_clipboard(snapshot);
...
pub(crate) async fn test_paste(request_id: String, state: &AppState) -> Response {
    ...
    let Some(backend) = skald_platform::paste_backend() else {
        return state_error(
            request_id,
            state,
            "paste_test_unavailable",
    ...
    if backend != skald_platform::PasteBackend::Hyprland && target.is_terminal() {
        return state_error(
            request_id,
            state,
            "paste_test_unavailable",
    ...
    let result = tokio::task::spawn_blocking(move || {
        let snapshot = skald_platform::save_clipboard();
        skald_platform::copy_to_clipboard("Skald paste test")?;
        skald_platform::wait_for_clipboard(delay_ms);
        if skald_platform::capture_active_target().as_ref() != Some(&target_for_check) {
            return Err(skald_platform::PlatformError::InvalidOutput {
                tool: "paste",
                message: "active target changed before paste".into(),
            });
        }
        let paste_result = skald_platform::paste(backend);
        skald_platform::wait_for_clipboard(delay_ms);
        let restore_result = skald_platform::restore_clipboard(snapshot);
