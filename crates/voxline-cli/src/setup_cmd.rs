...
use voxline_platform::trigger_guidance;
...
pub async fn run(options: SetupOptions) -> Result<()> {
    if std::env::var("VOXLINE_SKIP_SETUP").as_deref() == Ok("1") {
        return Ok(());
    }
    ...
    if profile.has_nvidia_gpu && cuda_build != Some(true) && !options.non_interactive {
        println!("\nNVIDIA GPU detected but voxlined was not built with CUDA support.");
        ...
        let continue_cpu = Confirm::with_theme(&dialoguer::theme::ColorfulTheme::default())
            ...
            .interact()?;
        if !continue_cpu {
            bail!("setup cancelled; rebuild voxlined with CUDA support first");
        }
    }
    ...
    if options.json {
        ...
        println!("Run `voxline doctor` to verify the installation.");
    }
...
fn print_profile(profile: &SystemProfile, cuda_build: Option<bool>) {
    println!("VoxLine setup — system profile");
...
async fn ensure_daemon() -> Result<DaemonGuard> {
    ...
    let voxlined = find_voxlined()?;
    let child = ProcessCommand::new(voxlined)
        .arg("--foreground")
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
        .context("failed to start voxlined for setup")?;
    for _ in 0..40 {
        if send(Command::Status).await.is_ok() {
    ...
    bail!("voxlined did not become ready");
}

fn find_voxlined() -> Result<PathBuf> {
    if let Ok(path) = std::env::current_exe() {
        let sibling = path.with_file_name("voxlined");
        if sibling.is_file() {
            return Ok(sibling);
        }
    }
...
fn which_voxlined() -> Result<PathBuf> {
    let output = ProcessCommand::new("sh")
        .args(["-c", "command -v voxlined"])
        .output()
        .context("failed to locate voxlined")?;
    if !output.status.success() {
        bail!("voxlined not found in PATH; run `just install` or `just build-cuda` first");
    }
    Ok(PathBuf::from(
...
) -> Result<Vec<voxline_core::protocol::ModelBenchResult>> {
...
fn select_model(
    results: &[&voxline_core::protocol::ModelBenchResult],
