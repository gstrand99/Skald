...
use voxline_core::service::{self, SERVICE_UNIT_NAME};
...
pub fn install(log_level: &str) -> Result<()> {
    ...
    let voxlined = resolve_voxlined_path()?;
    service::write_service_unit(&unit_path, &voxlined.display().to_string(), log_level)
    ...
    println!("  voxline service start");
    println!();
    println!("Check status:");
    println!("  systemctl --user status {SERVICE_UNIT_NAME}");
    println!("  voxline service status");
    println!();
...
pub fn print_trigger_guidance() {
    let environment = voxline_platform::environment_report();
...
fn resolve_voxlined_path() -> Result<PathBuf> {
    let current = std::env::current_exe().context("failed to resolve current executable")?;
    let sibling = current.with_file_name("voxlined");
    if sibling.is_file() {
        return Ok(sibling);
    }
    let output = Command::new("sh")
        ...
        .output()
        .context("failed to search PATH for voxlined")?;
    ...
    bail!("could not find voxlined binary; build or install it next to voxline");
}
