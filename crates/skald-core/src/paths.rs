...
mod tests {
    ...
    fn expand_home_handles_dollar_home_prefix() {
        ...
        assert_eq!(
            expand_home("$HOME/.config/skald"),
            home.join(".config/skald")
        );
    ...
    fn to_tilde_round_trips_with_expand_home() {
        ...
        let path = home.join(".config/skald/config.toml");
        let tilde = to_tilde(&path, &home.join("models"), "~/models");
        assert_eq!(tilde, "~/.config/skald/config.toml");
        assert_eq!(expand_home(&tilde), path);
    }

    #[test]
    fn to_tilde_uses_model_dir_tilde_for_files_under_model_dir() {
        let base = std::env::temp_dir().join(format!("skald-paths-{}", ulid::Ulid::new()));
