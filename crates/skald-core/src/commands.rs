...
mod tests {
    ...
    #[test]
    fn parses_prefix_when_asr_splits_skald() {
        let base = std::env::temp_dir().join(format!("skald-commands-{}", ulid::Ulid::new()));
        ...
        let parsed = parse_voice_command(
            &enabled_config(),
            &registry,
            "Skald professional hey John thanks",
        )
        .unwrap();
        assert_eq!(
            parsed.target,
...
"#,
        ...
        let registry = build_command_registry(&paths).unwrap();
        let parsed =
            parse_voice_command(&enabled_config(), &registry, "skald insert signature").unwrap();
        assert_eq!(
            parsed.target,
