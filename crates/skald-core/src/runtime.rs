...
pub fn xdg_runtime_dir() -> Result<PathBuf, RuntimeError> {
    ...
    Ok(PathBuf::from(base).join("skald"))
}
...
pub fn socket_path_for(paths: &PathsConfig) -> Result<PathBuf, RuntimeError> {
    Ok(runtime_dir_for(paths)?.join("skaldd.sock"))
}
...
mod tests {
    ...
    fn verify_socket_rejects_group_permissions() {
        let socket = std::env::temp_dir().join(format!("skald-socket-test-{}", std::process::id()));
    ...
    fn verify_owner_accepts_current_user_file() {
        let path = std::env::temp_dir().join(format!("skald-owner-test-{}", std::process::id()));
