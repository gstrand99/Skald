...
Skald is a Linux-first, local-first dictation app inspired by the interaction model of Wispr Flow: trigger dictation, speak, stop, and safely insert the final text into the active application.
...
User triggers dictation
  ↓
skaldd starts recording microphone audio
  ↓
User stops dictation
  ↓
skaldd finalizes a temporary WAV in runtime storage
  ↓
local Whisper backend transcribes the full recording
  ↓
final transcript is copied to clipboard
  ↓
if safe, Skald attempts paste injection
  ↓
...
dictate doctor equivalent, named skald doctor
...
```text
Config:      ~/.config/skald/config.toml
Styles:      ~/.config/skald/styles/
Apps:        ~/.config/skald/apps/
Snippets:    ~/.config/skald/snippets/
Models:      ~/.local/share/skald/models/
Runtime:     $XDG_RUNTIME_DIR/skald/
Socket:      $XDG_RUNTIME_DIR/skald/skaldd.sock
Service:     ~/.config/systemd/user/skaldd.service
```
...
```bash
skald bench asr ./samples/10s.wav
skald bench end-to-end ./samples/10s.wav --no-cleanup
skald bench end-to-end ./samples/10s.wav --cleanup
...
Skald v1 does not implement native global hotkey capture inside the daemon.
...
```text
key press -> skald start
key release -> skald stop
```
...
```text
bindsym $mod+space exec skald toggle
```

Sway push-to-talk example:

```text
bindsym $mod+v exec skald start
bindsym --release $mod+v exec skald stop
```

X11 example:

```text
Bind a desktop shortcut to: skald toggle
```

### 7.5 Doctor trigger guidance

`skald doctor` must print platform-specific guidance:

```text
Trigger mode: external shortcut
Recommended command: skald toggle
Push-to-talk support: available if your compositor supports release bindings
```

`skald service install` must also print shortcut setup instructions.
...
```text
Unix domain socket at $XDG_RUNTIME_DIR/skald/skaldd.sock
```

Windows later:

```text
Named pipe: \\.\pipe\skald-skaldd
```
...
```json
{
  ...
  "error": {
    "code": "busy",
    "message": "Skald is currently transcribing"
  },
  "job_id": "01J...",
  "state": "Transcribing"
}
...
mix down to mono
resample to 16 kHz
write WAV under $XDG_RUNTIME_DIR/skald/<job-id>.wav
...
```text
$XDG_RUNTIME_DIR/skald/<job-id>.wav
```
...
If debug retention is enabled, warn loudly in `skald doctor`.
...
Make replacements testable with skald vocab test.
```
...
```bash
skald vocab list
skald vocab add phrase "OpenRouter"
skald vocab add replace "open router" "OpenRouter"
skald vocab test "I configured open router in hyper land"
```
...
```text
You are the cleanup engine for Skald, a local dictation app.
...
```text
Require a trigger prefix such as "Skald professional" or "Skald signature".
...
Skald runs as a user-session process, not a system service.
...
exec-once = systemctl --user start skaldd
```

`skald service install` must print:
...
```bash
skald test mic
skald record --no-cleanup
skald transcribe <audio-file> --no-cleanup
skald bench asr <audio-file>
skald bench end-to-end <audio-file> --no-cleanup
```
...
skald vocab add phrase "OpenRouter"
skald vocab add replace "open router" "OpenRouter"
skald vocab test "configure open router in hyper land"
```
...
skald toggle --no-cleanup
skald toggle --style professional
```
...
`skald doctor` is required before production use.
...
$XDG_RUNTIME_DIR/skald can be created
mode is 0700
socket owned by current user
```
...
skald asr load
skald asr status
skald transcribe ./sample.wav --no-cleanup
skald bench asr ./sample.wav
skald vocab test "open router in hyper land"
```
...
```text
bind a key to skald toggle, speak, stop, and see local transcript inserted safely
```
...
```text
[ ] Create workspace with skald-core, skaldd, skald-cli, skald-platform.
...
[ ] Implement event subscription and skald watch.
